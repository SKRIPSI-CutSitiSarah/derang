...
import { ShieldAlertIcon, Loader2Icon, CheckCircle2Icon, MailIcon, LockIcon } from 'lucide-react'
    ...
    <MagicCard className="relative overflow-hidden border-border/50 bg-card/65 backdrop-blur-xl shadow-2xl text-card-foreground">
      ...
      <form action={formAction}>
        <CardContent className="space-y-4 px-6 pb-4">
          {isRegistered && (
            <div className="rounded-lg bg-emerald-500/10 p-3 text-xs text-emerald-600 border border-emerald-500/20 flex items-center gap-2 animate-pulse-slow">
              <CheckCircle2Icon className="h-4 w-4 text-emerald-600 shrink-0" />
              <p className="font-semibold">Registrasi berhasil! Silakan masuk dengan akun Anda.</p>
            </div>
          )}
          {state?.error && (
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-background px-4 py-12">
      ...
      <div className="w-full max-w-[400px] z-10 space-y-6">
        ...
        <Suspense fallback={
          <MagicCard className="border-border bg-card/60 backdrop-blur-xl shadow-2xl p-8 flex flex-col items-center justify-center min-h-[350px]">
